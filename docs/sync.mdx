...
Creates the remote folder, the local folder, and the `syncfolder` row pairing
them — the three steps that otherwise mean the CLI, Finder, and the app's
preferences pane.

Only the `syncfolder` row is written. `syncedfolder`, `localfolder` and
`localfile` are the daemon's own scan index; it derives them when it next
starts, and seeding them by hand would be inventing state its scanner is about
to compute.
...
| Remote paired  | That cloud folder already belongs to another pair                         |
| Root missing   | The derived root does not exist on disk                                   |

`--apply` backs up `data.db`, refuses while pCloud Drive is open, and offers to
quit and restart it — the same contract as `prune` and `settings`.
