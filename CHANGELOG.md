...
- **The Rewind tab reads as a history rather than a log.** It was two hundred rows each repeating `31 Jul 2026`, with the same file appearing forty times over because it had been saved every few minutes. A day's events on one file now fold into a single row — `21:13  ~ modified  ×12  /Backup/usage-kud.tsv  2m ago` — grouped under `Today`, `Yesterday` or a weekday heading. The right arrow opens a run into its individual saves when one particular moment is what you want; the left arrow closes it again.
