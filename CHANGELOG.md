...
- **The Shares row fits the pane it renders into.** The previous attempt sized
  the columns against the pane's full width, but a row starts five columns in —
  four for the selection marker, one for the list's padding — so the created
  date still arrived as `03 Nov 20…`. The layout now budgets for that chrome,
  and a test holds it there.
- **Six-digit share ids render whole.** pCloud issues ids like `225308`, and the
  id column had been sized against the four-digit ones in the fixtures. It is
  the one value on a share row you cannot infer from context, being the argument
  `remove-share` takes.
